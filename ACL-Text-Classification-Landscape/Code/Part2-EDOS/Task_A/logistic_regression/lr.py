"""
Logistic Regression Model for Binary Classification: Sexist vs Not Sexist
...
def train_model(X_train, y_train, C=1.0, max_iter=1000, random_state=42):
    """
    Train logistic regression model
    """
    print(f"\nTraining logistic regression model (C={C}, max_iter={max_iter})...")
...
def plot_f1_boxplot(all_results, save_path=None):
    ...
    if save_path:
        ...
        print(f"F1 bar plot saved to {bar_plot_path}")
    plt.close()


def get_top_features(model, vectorizer, n_features=20, class_idx=1):
    """
    Get top features for a given class
    class_idx: 0 for 'not sexist', 1 for 'sexist'
    """
    feature_names = vectorizer.get_feature_names_out()
    coefficients = model.coef_[0]
    
    if class_idx == 1:
        # For sexist class, get features with highest positive coefficients
        top_indices = coefficients.argsort()[-n_features:][::-1]
        class_name = "Sexist"
    else:
        # For not sexist class, get features with lowest (most negative) coefficients
        top_indices = coefficients.argsort()[:n_features]
        class_name = "Not Sexist"
    
    print(f"\nTop {n_features} features for {class_name} class:")
    print("-" * 60)
    for idx in top_indices:
        print(f"{feature_names[idx]:30s} {coefficients[idx]:8.4f}")
    
    return top_indices, feature_names, coefficients
...
def main():
    ...
    # Get top features
    get_top_features(model, vectorizer, n_features=30, class_idx=1)
    print("\n")
    get_top_features(model, vectorizer, n_features=30, class_idx=0)
