...
def extract_features(train_texts, dev_texts, test_texts, max_features=5000, ngram_range=(1, 2)):
    ...
    # Convert to dense arrays for Random Forest (RF requires dense arrays)
    print("Converting sparse matrices to dense arrays for Random Forest...")
...
def train_model(X_train, y_train, n_estimators=100, max_depth=None, 
    ...
    """
    Train Random Forest classifier for multi-class classification
    """
...
def find_optimal_thresholds(y_true, y_proba, class_names):
    """
    Find optimal thresholds for each class to maximize macro F1 score.
    Uses one-vs-rest approach: for each class, finds threshold that maximizes F1.
    """
    n_classes = len(class_names)
    optimal_thresholds = np.ones(n_classes) / n_classes  # Default: uniform thresholds
    
    print(f"\nFinding optimal thresholds on dev set...")
    ...
    for class_idx in range(n_classes):
        ...
        for threshold in thresholds:
            ...
            if y_pred_binary.sum() > 0:  # At least one positive prediction
        ...
        print(f"  {class_names[class_idx]:35s}: threshold={best_threshold:.3f}, F1={best_f1:.4f}")
...
def predict_with_thresholds(y_proba, thresholds):
    """
    Predict classes using custom thresholds.
    If multiple classes exceed their thresholds, choose the one with highest probability.
    If none exceed thresholds, choose argmax.
    """
    ...
    for i in range(n_samples):
        ...
        above_threshold = y_proba[i] >= thresholds
        if above_threshold.any():
            # Among classes above threshold, choose highest probability
            candidates = np.where(above_threshold)[0]
            y_pred[i] = candidates[np.argmax(y_proba[i, candidates])]
        else:
            # None above threshold, use argmax
            y_pred[i] = np.argmax(y_proba[i])
    
    return y_pred


def evaluate_model(model, X, y, class_names, split_name="", return_predictions=False, thresholds=None):
    """
    Evaluate model and return metrics for multi-class classification
    If thresholds are provided, use them for prediction; otherwise use default argmax
    """
    y_pred_proba = model.predict_proba(X)
    
    if thresholds is not None:
        y_pred = predict_with_thresholds(y_pred_proba, thresholds)
        print(f"\nUsing custom thresholds for {split_name} set")
    else:
...
def plot_feature_importance(model, vectorizer, top_n=30, save_path=None):
...
def save_model(model, vectorizer, label_encoder, save_dir="models"):
    ...
    vectorizer_path = os.path.join(save_dir, "tfidf_vectorizer_rf.pkl")
    encoder_path = os.path.join(save_dir, "label_encoder_rf.pkl")
...
def main():
    ...
    MAX_DEPTH = None  # None = unlimited depth, or set to integer
    MIN_SAMPLES_SPLIT = 2  # Minimum samples required to split a node
    MIN_SAMPLES_LEAF = 1  # Minimum samples required at a leaf node
    
    # More aggressive class weights for severe imbalance
    # Based on train set distribution: none=10602, threats=310, derogation=1590, animosity=1165, prejudiced=333
    # Weight inversely proportional to frequency (more aggressive than 'balanced')
    CLASS_WEIGHT = {
        0: 1.0,      # none (majority)
        1: 34.2,     # threats (most rare - 10602/310 ≈ 34.2)
        2: 6.67,     # derogation (10602/1590 ≈ 6.67)
        3: 9.10,     # animosity (10602/1165 ≈ 9.10)
        4: 31.84     # prejudiced (10602/333 ≈ 31.84)
    }
    
    # SMOTE Configuration
    USE_SMOTE = True  # Set to True to use SMOTE oversampling
    # Specify target number of samples for each minority class
    # This creates more balanced data without excessive oversampling
    # Only specify minority classes to oversample
    SMOTE_SAMPLING_STRATEGY = {
        1: 1000,  # Threats: target 1000 samples (from 310)
        2: 3000,  # Derogation: target 3000 samples (from 1590)
        3: 3000,  # Animosity: target 3000 samples (from 1165)
        4: 1000   # Prejudiced: target 1000 samples (from 333)
    }
    ...
    if USE_SMOTE:
        ...
        # Create SMOTE instance with custom sampling strategy
        smote = SMOTE(
            sampling_strategy=SMOTE_SAMPLING_STRATEGY,
            random_state=42,
            k_neighbors=5  # Number of neighbors to use for SMOTE
        )
    ...
    plot_feature_importance(model, vectorizer, top_n=30,
                           save_path=os.path.join(OUTPUT_DIR, "feature_importance.png"))
    
    # Evaluate on train and dev sets (default predictions)
    ...
    print(f"{'='*60}")
    test_results_default = evaluate_model(model, X_test, y_test, CLASS_NAMES, "Test (Default)", return_predictions=True)
    test_results = evaluate_model(model, X_test, y_test, CLASS_NAMES, "Test (Optimized Thresholds)", 
                                  return_predictions=True, thresholds=optimal_thresholds)
    ...
    plot_confusion_matrix(y_test, test_results_default['predictions'], CLASS_NAMES, "Test (Default)",
                         save_path=os.path.join(OUTPUT_DIR, "confusion_matrix_test_default.png"))
    plot_confusion_matrix(y_test, test_results['predictions'], CLASS_NAMES, "Test (Optimized Thresholds)",
                         save_path=os.path.join(OUTPUT_DIR, "confusion_matrix_test_optimized.png"))
    ...
    # Save metrics to JSON (with optimized thresholds)
    ...
    print(f"{'='*60}")
    print(f"Test Set - Default vs Optimized Thresholds:")
    print(f"  Macro F1:        {test_results_default['f1_macro']:.4f} → {test_results['f1_macro']:.4f} ({test_results['f1_macro'] - test_results_default['f1_macro']:+.4f})")
    print(f"  Macro Recall:    {test_results_default['recall_macro']:.4f} → {test_results['recall_macro']:.4f} ({test_results['recall_macro'] - test_results_default['recall_macro']:+.4f})")
    print(f"  Accuracy:        {test_results_default['accuracy']:.4f} → {test_results['accuracy']:.4f} ({test_results['accuracy'] - test_results_default['accuracy']:+.4f})")
    print(f"\nPer-class F1 improvement:")
    for i, class_name in enumerate(CLASS_NAMES):
        default_f1 = test_results_default['f1_per_class'][i]
        optimized_f1 = test_results['f1_per_class'][i]
        improvement = optimized_f1 - default_f1
        print(f"  {class_name:35s}: {default_f1:.4f} → {optimized_f1:.4f} ({improvement:+.4f})")
